...
public class ART2aClusteringTest {
    ...
    @BeforeAll
    public static void startClustering() throws IOException, ExecutionException {
        ...
        for(float i = 0.1f; i<1.0f; i += 0.1f) {
            Art2aClusteringTask task = new Art2aClusteringTask(i, dataMatrix,100);
            tmpClusteringTask.add(task);
        }
        FileUtil util = new FileUtil();
        PrintWriter writer = util.listToFile();
        ...
        List<Future<ART2aClustering>> tmpFuturesList;
         try {
            ...
            for (Future<ART2aClustering> tmpFuture : tmpFuturesList) {
                ...
                result.getResult();
               // System.out.println(result.deneme() + "-----deneme");
                for(String v : result.getResult()) {
                    writer.println(v);

                }
               // System.out.println(result.getListeEnd() + "-------yeniiiiiiiii");
                /*
                for(int z = 0; z<result.getListeEnd().size(); z++) {
                    for(String h : result.getListeEnd().get(z)) {
                        writer.println(h);
                    }
                }
                */
